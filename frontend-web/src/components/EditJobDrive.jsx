...
const EditJobDrive = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    // Selection Process - Use selectionRounds array instead
    selectionRounds: [
      {
        name: "",
        details: "",
  ...
  const fetchJobDrive = async () => {
    try {
      ...
      setFormData({
        ...
        // Selection Process - Handle both 'rounds' and 'selectionRounds'
        rounds: Array.isArray(drive.rounds) && drive.rounds.length > 0 
          ? drive.rounds.join(", ") 
          : Array.isArray(drive.selectionRounds) 
            ? drive.selectionRounds.map(round => round.name).join(", ")
            : "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        // Selection Process - Send as both formats
        rounds: formData.rounds
          .split(",")
          .map((s) => s.trim())
          .filter((s) => s),
        selectionRounds: formData.rounds
          .split(",")
          .map((s) => s.trim())
          .filter((s) => s)
          .map(roundName => ({
        ...
        <div className="bg-white rounded-lg shadow-md">
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-8">
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              ...
              {/* CTC Breakdown */}
              <div className="mt-4">
                <h3 className="text-md font-medium mb-2">CTC Breakdown (Optional)</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
              ...
              <div className="space-y-2">
                <label className="flex items-center">
                  <input
                    ...
                    className="mr-2"
                  />
                  <span className="text-sm">Mark as Dream Job</span>
                </label>
                <label className="flex items-center">
                  <input
                    type="checkbox"
