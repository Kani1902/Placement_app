...
const ManageDrives = () => {
  ...
  const isDriveEnded = (drive) => {
    ...
    if (drive.time) {
      try {
        ...
        driveDate.setHours(parseInt(hours), parseInt(minutes), 0, 0);
      } catch (error) {
        console.error('Error parsing time:', drive.time);
  ...
  const fetchDepartmentJobDrives = async (department) => {
    try {
      console.log('=== FETCHING DEPARTMENT DRIVES ===');
      ...
      console.log('Department drives response:', response.data);
      console.log('Department drives count:', response.data.jobDrives?.length || 0);
      
      // Also check if SLB is in all drives and why it's not in department filter
      const allDrivesForDebug = drives.filter(drive => {
        const eligibility = drive.eligibility || {};
        console.log(`Drive: ${drive.companyName}`);
        console.log('Allowed departments:', eligibility.allowedDepartments);
        console.log('Department length:', eligibility.allowedDepartments?.length);
        console.log('Includes selected dept:', eligibility.allowedDepartments?.includes(department));
        console.log('No restrictions (empty array):', !eligibility.allowedDepartments || eligibility.allowedDepartments.length === 0);
        ...
        if (!eligibility.allowedDepartments || eligibility.allowedDepartments.length === 0) {
          console.log('✅ No restrictions - showing to all departments');
          return true;
        }
        // Check if selected department is in allowed departments
        const isAllowed = eligibility.allowedDepartments.includes(department);
        console.log(isAllowed ? '✅ Department allowed' : '❌ Department not allowed');
        return isAllowed;
      });
      
      console.log('Filtered drives for department:', allDrivesForDebug.length);
  ...
  const downloadJobDriveStudentsCSV = () => {
    ...
    const headers = ['S.No', 'Name', 'Roll Number', 'Department', 'Email', 'CGPA', 'Mobile', 'Applied On'];
    const csvContent = [
      headers.join(','),
      ...selectedJobDriveStudents.map((student, index) => [
        ...
        `"${student.email}"`,
        `"${student.cgpa || 'N/A'}"`,
        `"${student.phoneNumber || student.phone || student.mobileNumber || 'N/A'}"`,
        `"${student.appliedAt ? new Date(student.appliedAt).toLocaleDateString() : 'N/A'}"`
      ].join(','))
    ].join('\n');
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-lg shadow mb-8">
          <div className="px-6 py-4 border-b border-gray-200">
            <h2 className="text-xl font-semibold text-gray-900">All Job Drives</h2>
          </div>
          
          {loading ? (
            <div className="text-center py-8">
              <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-500 mx-auto"></div>
              <p className="mt-4 text-gray-600">Loading job drives...</p>
            </div>
            ...
            <div className="divide-y divide-gray-200">
              {filteredDrives.map((drive) => {
                console.log('Rendering main drive:', drive.companyName, drive._id);
                return (
                  <div key={drive._id} className="p-6">
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm text-gray-500">
                          <div>
                            <span className="font-medium">Location:</span> {drive.location || drive.displayLocation || 'Not specified'}
                          </div>
                          <div>
                            <span className="font-medium">CTC:</span> 
                            {drive.ctc ? `₹${drive.ctc} LPA` : 'Not specified'}
                      ...
                      <div className="flex space-x-2 ml-4">
                        ...
                        {isDriveEnded(drive) ? (
                          ...
                          <button
                            ...
                            className="px-3 py-1 bg-green-100 text-green-700 hover:bg-green-200 rounded text-sm font-medium"
                          >
                            Applied Students ({drive.applications?.length || 0})
                          </button>
                        )}
                      </div>
                    </div>
